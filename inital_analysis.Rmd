...
withqol %>%
  ...
  scale_fill_grey(name = "Severity of disease")
...
withqol %>%
  ...
  withqol %>%
  mutate(fu12_fvc = as.numeric(fu12_fvcpred)) %>%
  ggplot(aes(x = fu12_fvc, y = PCS)) +
  geom_point() +
  xlab("FVC (predicted) %") +
  ylab("Physical Composite score") +
...
withqol %>%
  select(fu12_cxr, adm_news, adm_o2, adm_spo2, adm_lung.factor, fu_cxr_severity, composite_inc_niv, fu12_cxrreport) %>%
  tbl_summary()
